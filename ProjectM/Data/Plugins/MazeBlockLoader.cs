    ...
    public class MazeBlockLoader : IBlockGenerator
    {
        ...
        public Block CreateBlock(string name)
        {
            ...
            switch (part)
            {
                ...
                case "redLight":
                    return CreateBlockInfo(name, Block.VoxelSize / 5.0f, new Color4f(ObjBright, ObjDim, ObjDim, 1.0f), 1.0f, null,
                        new LightComponent(new Vector3b(bc, bc, bc), new Color3f(LightBright, LightDim, LightDim), ForFantasy ? 5 : 9), colorVariation: 0.0f);
                case "smallLight":
                    return CreateBlockInfo(name, Block.VoxelSize / 16.0f, new Color4f(ObjDim, ObjDim, ObjBright, 1.0f), 1.0f, null,
                        new LightComponent(new Vector3b(bc, bc, bc), new Color3f(LightDim, LightDim, LightBright), ForFantasy ? 12 : 20), colorVariation: 0.0f);
                case "hoverLightBlue":
                    return CreateBlockInfo(name, Block.VoxelSize / 4.0f, new Color4f(ObjDim, ObjDim, ObjBright, 1.0f), 1.0f, null,
                ...
                case "treeLight":
                    return CreateBlockInfo(name, Block.VoxelSize / 8.0f, new Color4f(ObjBright, ObjBright, ObjBright, 1.0f), 1.0f, null,
        ...
        {
            Block leaves = CreateRoundedBlockInfo(name, new Voxel(13, 200, 23), 0.05f, num, null, 0.3f);
        ...
        private static Block CreateGrass(string name, int lightDist = 2)
        {
            ...
            //const float bsmo = Block.VoxelSize - 1;

            //for (int bladeCount = 0; bladeCount < Block.VoxelSize * 8; bladeCount++)
            //{
            ...
            //    do
            //    {
            //        vX = random.Next(Block.VoxelSize);
            //        vY = random.Next(Block.VoxelSize);
            //        offsetX = random.NextFloat(true) * 6 - 3;
            //        offsetY = random.NextFloat(true) * 6 - 3;
            //    }
            //    while (vX + offsetX < 0.0f || vX + offsetX >= Block.VoxelSize || vY + offsetY < 0.0f || vY + offsetY >= Block.VoxelSize);
            ...
            //        if (vZ < bladeLength - 1)
            //        {
            //            vX += (offsetX / Block.VoxelSize);
            //            vY += (offsetY / Block.VoxelSize);
            //        }
            //    }

            //    if (bladeLength == Block.VoxelSize && random.NextFloat() < 0.2f && !putFlowerInBlock &&
            //        vX >= 1 && vX <= Block.VoxelSize - 2 && vY >= 1 && vY <= Block.VoxelSize - 2)
            //    {
            //        int fX = (int)vX;
            //        int fY = (int)vY;
            ...
            //        }
            //    }
            //    //int vX = random.Next(Block.VoxelSize);
            //    //int vY = random.Next(Block.VoxelSize);
            //    //for (int vZ = 0; vZ < bladeLength; vZ++)
            //    //{
            ...
            //    //        else if (rnd == 1 && vY > 0)
            //    //            vY--;
            //    //        else if (rnd == 2 && vX < Block.VoxelSize - 1)
            //    //            vX++;
            //    //        else if (rnd == 3 && vY < Block.VoxelSize - 1)
            //    //            vY++;
            //    //        else if (rnd != 4)
            //    //            tryAgain = true;
            //    //    }
                ...
                for (int x = 0; x < Block.VoxelSize; x++)
                {
                    for (int y = 0; y < Block.VoxelSize; y++)
                    {
                        if (random.NextFloat() < 0.3f - z / divisor && (z == 0 || GrassVoxelUnder(grass, x, y, z)))
                        {
                            ...
                            if (z == Block.VoxelSize - 1 && random.NextFloat() < 0.2f &&
